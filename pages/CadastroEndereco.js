...
export default function CadastroEndereco({ navigation, route }) {
  ...
  async function carregarDados() {
    try {
      ...
      setCidades(cidadesData.map(cidade => ({ label: cidade.nome, value: cidade.id })));
    } catch (err) {
  ...
  return (
    <SafeAreaView>
      ...
      <View style={styles.container}>
        ...
        <InputText label="Número" placeholder="Ex.: 123" value={numero} onChangeText={setNumero} keyboardType="numeric"/>
        <InputText label="Complemento (opcional)" placeholder="Ex.: Apartamento 202" value={complemento} onChangeText={setComplemento}/>
        <InputText label="CEP" placeholder="Somente números" value={cep} onChangeText={setCep} keyboardType="numeric" />
        <InputSelect label="Cidade" selectedValue={cidade} onValueChange={setCidade} items={cidades} />
